...
const OppositePlayer = () => {
  ...
  const addPlayer = (e) => {
    ...
    const cleanOvers = bowlingOvers
      ...
      .map((over) => ({
        overNumber: Number(over.overNumber) || 0,
        runsInOver: Number(over.runsInOver) || 0,
        extrasInOver: Number(over.extrasInOver) || 0,
    ...
    const playerData = {
      ...
      sixers: form.sixes.value ? Number(form.sixes.value) : null,
      fours: form.fours.value ? Number(form.fours.value) : null,
      totalWicket: form.totalWicket.value ? Number(form.totalWicket.value) : null,
      totalBall: form.totalBall.value ? Number(form.totalBall.value) : null,
      ballsFaced: form.ballsFaced.value ? Number(form.ballsFaced.value) : null,
      isOut: outStatus,
      battingPosition: Number(battingPosition),
  ...
  const submitAllPlayers = async () => {
    try {
      await axios.post("http://localhost:5002/api/OppositePlayer/playerWithBowling1", players);
        ...
        <div id="form-container">
          <div id="match-details" className="player-form">
            ...
            <form id="matchForm" onSubmit={addPlayer}>
              <label>
                MatchCode*
                <input type="text" name="matchcode" required defaultValue={matchCode} />
              </label>
              ...
              {outStatus === true && (
                <>
                  ...
                  <input type="text" name="outOverBall" placeholder="Wicket at: e.g. 4.2" />
                </>
              )}

              <label>Runs<input type="number" name="runs" /></label>
              <label>Fours<input type="number" name="fours" /></label>
              <label>Sixes<input type="number" name="sixes" /></label>
              <label>Balls Faced<input type="number" name="ballsFaced" /></label>
              <label>Total Wickets<input type="number" name="totalWicket" /></label>
              <label>Total Balls Bowled<input type="number" name="totalBall" /></label>
              <label>Record<textarea name="record" maxLength={194} /></label>

              {bowlingOvers.map((over, index) => (
                <div key={index}>
                  <label>
                    <legend style={{ textAlign: "center" }}>Bowling Overs</legend>
                    Over Number:
                    <input
                      type="number"
                      name="overNumber"
